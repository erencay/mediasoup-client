...
const logger = new Logger('Chrome49');

const SCTP_NUM_STREAMS = { OS: 1024, MIS: 1024 };

export class Chrome49 extends HandlerInterface
{
	// Handler direction.
	...
	async restartIce(iceParameters: IceParameters): Promise<void>
	{
		...
		{
			const offer = await new Promise<RTCSessionDescription>((resolve, reject) =>
			{
				this._pc.createOffer(resolve, reject, { iceRestart: true });
			});
		...
		else
		{
			...
			{
				this._pc.createAnswer(resolve, reject);
			});
	...
	): Promise<HandlerSendResult>
	{
		...
		let offer = await new Promise<RTCSessionDescription>((resolve, reject) =>
		{
			this._pc.createOffer(resolve, reject);
		});
		...
		if (track.kind === 'video' && encodings && encodings.length > 1)
		{
			...
			offer = { type: 'offer', sdp: sdpTransform.write(localSdpObject) } as RTCSessionDescription;
		}
	...
	): Promise<HandlerReceiveResult>
	{
		...
		await this._pc.setLocalDescription(answerSdp);
